...
class Game:
    def __init__(self, scenes):
        ...
        self.ripples = []; self.freeze_ripples = []; self.freeze_emitters = []; self.phoenix_waves = []
    ...
    def draw_setup(self):
        ...
        def layout_row(count, item_w, label_w):
            usable = max(0, self.window_width - margin_x * 2 - label_w - 12)
            if count <= 1:
                gap = 0
                total = item_w
            else:
                gap = max(12, (usable - item_w * count) / (count - 1))
        ...
        self.screen.blit(label_map, (margin_x, map_y + 16))
        map_row_h = 50
        map_available = self.window_width - margin_x * 2 - label_map.get_width() - 12
        map_total = len(map_labels) * 140 + max(12, (map_available - 140 * len(map_labels)) / max(len(map_labels) - 1, 1)) * (len(map_labels) - 1)
        ...
        map_xs = layout_row(len(map_labels), 140, label_map.get_width())
        ...
        for (label, idx), x in zip(map_labels, map_xs):
            rect = pygame.Rect(x, map_y, 140, 50)
        ...
        self.screen.blit(label_count, (margin_x, count_y + 12))
        counts = list(range(2, 9))
        count_row_h = 44
        count_available = self.window_width - margin_x * 2 - label_count.get_width() - 12
        count_total = len(counts) * 60 + max(12, (count_available - 60 * len(counts)) / max(len(counts) - 1, 1)) * (len(counts) - 1)
        ...
        count_xs = layout_row(len(counts), 60, label_count.get_width())
        ...
        for c, x in zip(counts, count_xs):
            rect = pygame.Rect(x, count_y, 60, 44)
        ...
        if self.setup_row_active == "count" and count_overflow > 0:
            ...
            slider_y = count_y + count_row_h + 6
            pygame.draw.rect(self.screen, THEME["grid"], (track_left, slider_y, track_w, 6), border_radius=3)
            pygame.draw.rect(self.screen, THEME["accent"], (knob_x, slider_y-2, knob_w, 10), border_radius=4)
        ...
        self.screen.blit(label_time, (margin_x, time_y + 10))
        ...
        time_total = len(time_opts) * 100 + max(12, (time_available - 100 * len(time_opts)) / max(len(time_opts) - 1, 1)) * (len(time_opts) - 1)
        ...
        time_xs = layout_row(len(time_opts), 100, label_time.get_width())
        ...
        for t, x in zip(time_opts, time_xs):
            rect = pygame.Rect(x, time_y, 100, 44)
        ...
        self.screen.blit(label_env, (margin_x, env_y + 10))
        env_keys = ["none", "poison", "electric", "thunder", "overload", "random"]
        env_row_h = 44
        env_available = self.window_width - margin_x * 2 - label_env.get_width() - 12
        env_total = len(env_keys) * 140 + max(12, (env_available - 140 * len(env_keys)) / max(len(env_keys) - 1, 1)) * (len(env_keys) - 1)
        ...
        env_xs = layout_row(len(env_keys), 140, label_env.get_width())
        ...
        for key, x in zip(env_keys, env_xs):
            rect = pygame.Rect(x, env_y, 140, 44)
        ...
        # 开始按钮
        start_rect = pygame.Rect(cx - 80, env_y + 80, 160, 50)
