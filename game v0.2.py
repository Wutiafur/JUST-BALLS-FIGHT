...
# 可选地图（尺寸 / 形状）
MAPS = [
    {"name": "标准方形", "width": 800, "height": 600, "shape": "rect", "border_margin": 40},
...
PROFS_INFO = [
    ...
    ("无人机", "周身环绕一个橄榄绿三角，每8秒随机方向穿越全场，对经过的其他实体造成1点伤害后返回。"),
]
...
class Game:
    def __init__(self, scenes):
        pygame.init()
        pygame.display.set_caption("自主移动 + 几何回弹 + 倒计时 Demo")
    ...
    def assign_profession(self, ent):
        ...
        ent.drone_cd = DRONE_CD
        ent.drone_phase = "orbit" if prof == "drone" else None
        ent.drone_angle = random.uniform(0, 2 * math.pi)
        ent.drone_pos = (0, 0)
        ent.drone_dir = (0, 0)
        ent.drone_hit_ids = set()
    # ------ 工具：返回当前所有实体 ------ #
    ...
    def apply_power_up(self, ent):
        modes = ["aura", "bullet", "spike", "flash", "heal", "sweep", "aoe", "trail", "ripple", "shield", "wall"]
    ...
    def draw_drones(self):
        for ent in self.all_entities():
            if ent.profession != "drone":
                continue
            px, py = ent.drone_pos if ent.drone_pos else self.get_entity_center(ent)
            r = DRONE_RADIUS
            p1 = (int(px), int(py - r))
    ...
    def update_professions(self, dt):
        ...
        for ent in entities:
            ...
            if p == "assassin":
                ...
                cx, cy = self.get_entity_center(ent)
                if ent.drone_phase is None:
                    ent.drone_phase = "orbit"
                if ent.drone_phase == "orbit":
                    ent.drone_cd -= dt
                    ent.drone_angle = (ent.drone_angle + dt * 2.0) % (2 * math.pi)
                    offset = ent.radius + 14
                    ent.drone_pos = (cx + math.cos(ent.drone_angle) * offset, cy + math.sin(ent.drone_angle) * offset)
                    if ent.drone_cd <= 0:
                        ang = random.uniform(0, 2 * math.pi)
                        ent.drone_dir = (math.cos(ang), math.sin(ang))
                        ent.drone_phase = "fly"
                        ent.drone_hit_ids = set()
                elif ent.drone_phase == "fly":
                    dx, dy = ent.drone_dir
                    px, py = ent.drone_pos
                    ...
                    for target in entities:
                        if target is ent or not target.alive:
                            continue
                        if id(target) in ent.drone_hit_ids:
                            continue
                        tx, ty = self.get_entity_center(target)
                        if (tx - px) ** 2 + (ty - py) ** 2 <= (target.radius + DRONE_RADIUS) ** 2:
                            target.take_damage()
                            ent.drone_hit_ids.add(id(target))
                    bounds = self.get_bounds()
                    out = False
                    if bounds["shape"] == "rect":
                        ...
                        if (px - bx) ** 2 + (py - by) ** 2 > (r + 10) ** 2:
                            out = True
                    if out:
                        ent.drone_phase = "return"
                elif ent.drone_phase == "return":
                    px, py = ent.drone_pos
                    dx = cx - px
                    dy = cy - py
                    dist = math.hypot(dx, dy)
    ...
    def draw_menu(self):
        ...
        title = self.font.render("选择模式 / 查看职业（上下键），回车确认", True, (230, 230, 230))
        ...
        for i, sc in enumerate(self.scenes):
            ...
            self.screen.blit(surf, (60, base_y + i * 30))

        # 职业入口
        prof_idx = len(self.scenes)
        prof_color = (255, 255, 0) if self.scene_index == prof_idx else (200, 200, 200)
        prof_surf = self.font.render(f"{prof_idx + 1}. 职业介绍", True, prof_color)
        self.screen.blit(prof_surf, (60, base_y + len(self.scenes) * 30))

        hint = self.font.render("ESC 退出", True, (160, 160, 160))
        self.screen.blit(hint, (40, base_y + (len(self.scenes) + 1) * 30))
